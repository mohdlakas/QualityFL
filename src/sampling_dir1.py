...
def cifar10_dirichlet(dataset, num_users, alpha=0.5, min_samples_per_user=20):
    """
    Enhanced version of your existing cifar10_dirichlet function.
    Handles extreme heterogeneity without breaking DataLoaders.
    """
    # Auto-adjust minimum samples based on alpha to prevent failures
    if alpha <= 0.05:
        min_samples_per_user = max(min_samples_per_user, 80)
    elif alpha <= 0.1:
        min_samples_per_user = max(min_samples_per_user, 60)
    elif alpha <= 0.3:
        min_samples_per_user = max(min_samples_per_user, 40)
    ...
    min_total_needed = num_users * min_samples_per_user
    if min_total_needed > total_samples:
        print(f"Warning: Reducing min_samples from {min_samples_per_user} to {total_samples // num_users}")
        min_samples_per_user = max(10, total_samples // num_users)
    
    dict_users = {i: np.array([], dtype=np.int32) for i in range(num_users)}
    class_indices = [np.where(labels == c)[0] for c in range(num_classes)]
    
    # STEP 1: Ensure each user gets at least some samples from each class
    reserved_samples = set()
    base_samples_per_class = max(1, min_samples_per_user // (2 * num_classes))
    
    for user_idx in range(num_users):
        for class_idx in range(num_classes):
            available_in_class = np.setdiff1d(class_indices[class_idx], list(reserved_samples))
            if len(available_in_class) == 0:
                continue
                
            to_take = min(base_samples_per_class, len(available_in_class))
            if to_take > 0:
                selected = np.random.choice(available_in_class, to_take, replace=False)
                dict_users[user_idx] = np.concatenate([dict_users[user_idx], selected])
                reserved_samples.update(selected)
    
    # STEP 2: Distribute remaining samples using Dirichlet (same as your original logic)
    remaining_indices = np.setdiff1d(np.arange(total_samples), list(reserved_samples))
    ...
    if len(remaining_indices) > 0:
        ...
        for class_idx, class_remaining in remaining_by_class.items():
            if not class_remaining:
                continue
                
            class_remaining = np.array(class_remaining)
            np.random.shuffle(class_remaining)
            
            # Use your original Dirichlet logic but with safety bounds
            safe_alpha = max(alpha, 0.01)  # Prevent complete zeros
            proportions = np.random.dirichlet([safe_alpha] * num_users)
            counts = (proportions * len(class_remaining)).astype(np.int32)
            
            # Handle rounding (same as your original)
            diff = len(class_remaining) - counts.sum()
            if diff > 0:
            ...
            # Distribute samples (same as your original)
            start_idx = 0
            for user_idx in range(num_users):
                if counts[user_idx] > 0:
                    end_idx = start_idx + counts[user_idx]
                    user_samples = class_remaining[start_idx:end_idx]
                    dict_users[user_idx] = np.concatenate([dict_users[user_idx], user_samples])
                    start_idx = end_idx
    
    # STEP 3: Final safety check (replaces your ensure_min_samples call)
    for user_idx in range(num_users):
        if len(dict_users[user_idx]) < min_samples_per_user:
            deficit = min_samples_per_user - len(dict_users[user_idx])
            all_used = np.concatenate(list(dict_users.values())) if dict_users else np.array([], dtype=np.int32)
            available = np.setdiff1d(np.arange(total_samples), all_used)
            
            if len(available) >= deficit:
                additional = np.random.choice(available, deficit, replace=False)
                dict_users[user_idx] = np.concatenate([dict_users[user_idx], additional])
...
def cifar100_dirichlet(dataset, num_users, alpha=0.5, min_samples_per_user=30):
    """
    Enhanced CIFAR-100 version with higher minimum samples due to 100 classes.
    """
    # Auto-adjust for CIFAR-100's complexity
    if alpha <= 0.05:
        min_samples_per_user = max(min_samples_per_user, 150)
    elif alpha <= 0.1:
        min_samples_per_user = max(min_samples_per_user, 100)
    elif alpha <= 0.3:
        min_samples_per_user = max(min_samples_per_user, 60)
    
    # Use same logic as CIFAR-10 but with adjusted parameters
    return cifar10_dirichlet(dataset, num_users, alpha, min_samples_per_user)
...
def mnist_dirichlet(dataset, num_users, alpha=0.5, min_samples_per_user=10):
    """
    Enhanced MNIST version.
    """
    # MNIST is simpler, so lower minimums
    if alpha <= 0.05:
        min_samples_per_user = max(min_samples_per_user, 50)
    elif alpha <= 0.1:
        min_samples_per_user = max(min_samples_per_user, 40)
    elif alpha <= 0.3:
        min_samples_per_user = max(min_samples_per_user, 25)
