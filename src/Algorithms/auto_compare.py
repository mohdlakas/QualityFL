...
def run_experiment(script_name, method_name, args_dict):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"DEBUG {method_name}: Got {len(test_accuracies)} test accuracies")
            if test_accuracies:
                print(f"DEBUG {method_name}: First test acc: {test_accuracies[0]:.4f}, Last test acc: {test_accuracies[-1]:.4f}")
            else:
                print(f"DEBUG {method_name}: No test accuracies extracted!")
                # DEBUG: Print first few lines to see the actual output format
                output_lines = result.stdout.split('\n')
                print("DEBUG: First 10 lines of output:")
                for i, line in enumerate(output_lines[:10]):
                    if line.strip():
                        print(f"  {i}: {line}")

            # Extract final test accuracy with multiple fallbacks
            final_test_acc = None

            # Method 1: Use last test accuracy from curves
            if test_accuracies:
                final_test_acc = test_accuracies[-1]
                print(f"DEBUG {method_name}: Using last test accuracy: {final_test_acc:.4f}")

            # Method 2: Search for explicit "Final Test Accuracy" line
                ...
                for line in output_lines:
                    if "Final Test Accuracy" in line:
                        ...
                        if match:
                            ...
                            print(f"DEBUG {method_name}: Found explicit final accuracy: {final_test_acc:.4f}")
                            break

            # Method 3: Fallback to a reasonable default
            if final_test_acc is None:
                final_test_acc = 0.10  # 10% as explicit fallback
                print(f"WARNING {method_name}: No accuracy found, using fallback: {final_test_acc:.4f}")
...
def parse_training_curves(output_text, method_name):
    ...
    # DEBUG: Print lines that contain "Round" to see the format
    print(f"DEBUG {method_name}: Looking for lines with 'Round'...")
    found_round_lines = 0
    
    for line in lines:
        if "Round" in line and "Train Accuracy" in line and "Test Accuracy" in line:
            found_round_lines += 1
            if found_round_lines <= 3:  # Print first 3 matches
                print(f"DEBUG {method_name}: Found round line: '{line.strip()}'")
            ...
            if train_match and test_match:
                if found_round_lines <= 3:  # Debug first 3
                    print(f"DEBUG {method_name}: Extracted Train={train_match.group(1)}%, Test={test_match.group(1)}%")
                
                train_accuracies.append(float(train_match.group(1)) / 100.0)
                test_accuracies.append(float(test_match.group(1)) / 100.0)
                
                if loss_match:
                    train_losses.append(float(loss_match.group(1)))
            else:
                if found_round_lines <= 3:
                    print(f"DEBUG {method_name}: Regex failed for line: '{line.strip()}'")
