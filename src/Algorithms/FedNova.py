...
class FedNovaLocalUpdate(LocalUpdate):
    ...
    def update_weights_fednova(self, model, global_round):
        ...
        if gradient_sqnorm > 0:
            coeff = effective_tau / (1.0 + self.args.gm * gradient_sqnorm)
        else:
            coeff = effective_tau
        
        # Normalize the update# Replace the update_weights_fednova method:
    ...
    def update_weights_fednova(self, model, global_round):
        ...
        # FIXED: Initialize gradient norm per round, not accumulating
        total_norm = 0.0
        total_steps = 0
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                # FIXED: Calculate gradient norm per step, not accumulating
                step_norm = 0.0
                for param in model.parameters():
                    if param.grad is not None:
                        step_norm += param.grad.data.norm(2) ** 2
        ...
        # FIXED: Average gradient norm, not cumulative
        avg_gradient_norm = total_norm / total_steps if total_steps > 0 else 0.0
        
        # Calculate effective local steps
        effective_tau = self.tau
        w_new = model.state_dict()
        
        # FIXED: Use averaged gradient norm and adjust scaling
        if avg_gradient_norm > 0:
            # Scale down the gradient norm impact
            normalized_grad_norm = avg_gradient_norm / 1000.0  # Scale down
            coeff = effective_tau / (1.0 + self.args.gm * normalized_grad_norm)
        else:
            coeff = effective_tau
        
        # Add debug output for first few rounds
        if global_round < 3:
            print(f"DEBUG Round {global_round+1}: tau={effective_tau}, avg_grad_norm={avg_gradient_norm:.6f}, coeff={coeff:.6f}")
