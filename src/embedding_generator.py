...
class EmbeddingGenerator:
    ...
    def generate_embedding_from_stats(self, stats):
        """Generate embedding from pre-computed statistical features."""
        # Extract features in the same order as _extract_features
        features = np.array([
            stats.get('mean', 0.0),
            stats.get('std', 0.0),
            stats.get('l2_norm', 0.0),
            stats.get('l1_norm', 0.0),
            stats.get('min', 0.0),
            stats.get('max', 0.0),
            stats.get('median', 0.0),
            0.0,  # skewness (would need raw data)
            0.0,  # iqr (would need raw data)
            stats.get('mean', 0.0),  # mean absolute value (approximation)
            0.0,  # kurtosis (would need raw data)
            stats.get('sparsity', 0.0),
            stats.get('energy', 0.0)
        ])
        ...
        # Convert to tensor and process same as generate_embedding
        features = torch.tensor(features, dtype=torch.float32)
        features = torch.nan_to_num(features, nan=0.0, posinf=1.0, neginf=-1.0)
        features = self._scale_features(features)
        ...
        client_hash = hash(str(features.numpy().tolist())) % 10000
        torch.manual_seed(client_hash)
        noise = torch.normal(0, 0.05, size=embedding.shape)
        embedding = embedding + noise
