...
class QualityMetric:
    ...
    def __init__(self, alpha=0.5, beta=0.1, gamma=0.4):  # FIXED: More balanced weights
        """
...
class StableQualityMetric(QualityMetric):
    ...
    def calculate_quality(self, loss_before, loss_after, data_sizes, param_update, 
        ...
        if round_num < 15:
            ...
            self.logger.info(f"R{round_num} Client {client_id}: EXPLORATION MODE, "
                           f"quality={exploration_quality:.4f}")
            return exploration_quality
        ...
        # PHASE 3: Full quality-based selection (rounds 30+)
        else:
    ...
    # Alternative: Even more conservative version for debugging
class UltraConservativeQualityMetric(QualityMetric):
    """
    DEBUGGING VERSION: Minimal differences between clients to isolate issues
    """
    
    def __init__(self):
        super().__init__(alpha=0.33, beta=0.33, gamma=0.34)  # Perfectly balanced
        
    def calculate_quality(self, loss_before, loss_after, data_sizes, param_update, 
                         round_num, client_id, all_loss_improvements=None):
        """
        ULTRA-CONSERVATIVE: Minimal quality differences for debugging
        """
        
        # For first 20 rounds, use almost uniform quality
        if round_num < 20:
            return 0.5 + 0.05 * np.random.random()  # 0.5 to 0.55
            
        # After round 20, use very small quality differences
        base_quality = super().calculate_quality(
            loss_before, loss_after, data_sizes, param_update,
            round_num, client_id, all_loss_improvements
        )
        
        # Compress quality range to be very narrow
        compressed_quality = 0.45 + 0.1 * base_quality  # Range: 0.45 to 0.55
